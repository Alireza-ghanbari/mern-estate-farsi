...
export default function Search() {
  ...
  const onShowMoreClick = async () => {
    ...
    const satrtIndex = numOfListing;
    const urlParams = new URLSearchParams(location.search);
    urlParams.set("startIndex", satrtIndex);
    const searchQuery = urlParams.toString();
    const res = await fetch(`/api/listing/get?${searchQuery}`);
    const data = await res.json();
    if (data.length < 9) {
    ...
    <div className="flex flex-col lg:flex-row pb-24 lg:pb-0">
      <div className=" p-3 xs:p-7 border-b-2 lg:border-r-2 lg:min-h-screen">
        <form className="flex flex-col gap-8" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <option value="regularPrice_desc">بیشترین قیمت</option>
              <option value="regularPrice_asc">کمترین قیمت</option>
      ...
      <div className="flex-1 lg:pb-12">
        ...
        <div className="sm:p-7 p-3 flex flex-wrap gap-6">
          ...
          {showMore && (
            <button
              className="text-green-700 hover:underline p-1 mt-7 text-center w-full"
